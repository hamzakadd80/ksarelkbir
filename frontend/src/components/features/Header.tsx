...
const NAV_ITEMS = [
  ...
  { label: 'Carte', href: '/carte', icon: MapIcon },
  { label: '9asrawa', href: '/kasrawa', icon: Users },
  { label: 'Besoins', href: '/besoins', icon: HeartHandshake },
  { label: 'Offres', href: '/offres', icon: Gift },
...
export default function Header() {
  ...
  return (
    <>
      ...
      <div className="bg-red-600 text-white py-2 px-4 flex justify-center items-center gap-3 text-xs md:text-sm font-bold text-center relative z-[60]">
        <span className="animate-pulse bg-white/20 p-1 rounded-full">
            <AlertTriangle className="h-4 w-4" />
        </span>
        <span>Urgence Inondations Loukkos - Secours : <strong>15</strong></span>
      </div>
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <Link href="/" className="flex items-center gap-3 group">
              ...
              <div className="flex flex-col leading-none">
                <span className="text-lg font-bold text-slate-900 tracking-tight">Ksar</span>
                <span className="text-lg font-bold text-[#0f4c81]">Entraide</span>
              </div>
            </Link>
            ...
            <div className="hidden lg:flex items-center gap-3">
              <button className="flex items-center gap-2 px-3 py-2 text-slate-600 hover:bg-slate-50 rounded-lg transition-colors text-sm font-medium">
                <Globe size={18} />
                <span>العربية</span>
              </button>
      ...
      >
        ...
        {/* Panneau latéral */}
        <div className={`
          absolute right-0 top-0 bottom-0 w-[80%] max-w-sm bg-white shadow-2xl flex flex-col pt-24 px-6 gap-6 transition-transform duration-300 ease-out
        ...
        `}>
          ...
          <div className="mt-auto mb-8 flex flex-col gap-4">
             <button className="flex items-center justify-center gap-2 w-full py-3 border border-slate-200 rounded-xl text-slate-700 font-bold hover:bg-slate-50 transition-colors">
              <Globe size={20} />
              <span>Changer la langue (العربية)</span>
            </button>
