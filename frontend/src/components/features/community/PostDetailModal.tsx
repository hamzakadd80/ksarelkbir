...
// ... (imports remain the same, just adding useLanguage)
          ...
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0 z-10 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{t.community_page.author_badge}</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-white">
            {rootComments.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-slate-400 opacity-60">
                ...
                <p className="text-sm">{t.community_page.no_comments}</p>
              </div>
            ) : (
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-1 ml-2">
                        ...
                        <button onClick={() => setReplyingTo(comment)} className="text-xs font-bold text-slate-500 hover:text-emerald-600">{t.community_page.reply}</button>
                      </div>
                    </div>
                  </div>
          ...
          <div className="p-3 border-t border-slate-100 bg-white">
            {replyingTo && (
              <div className="flex justify-between items-center bg-emerald-50 px-3 py-1.5 rounded-lg mb-2 text-xs text-emerald-700 border border-emerald-100">
                <span>{t.community_page.replying_to} <b>{replyingTo.user.fullName}</b></span>
            ...
            <form onSubmit={handleSendComment} className="flex items-center gap-2">
              <input
                ...
                placeholder={replyingTo ? t.community_page.write_reply : t.community_page.write_comment}
