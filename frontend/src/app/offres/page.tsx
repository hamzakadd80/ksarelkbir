    ...
    <div className="min-h-screen bg-slate-50/50 py-12">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 mb-2">{t.offers_page.title}</h1>
            <p className="text-slate-500">
              <span className="font-bold text-sky-600">{offers.length} {t.offers_page.count_offers}</span>
            </p>
          </div>
          ...
          >
            ...
            <span>{t.offers_page.btn_add_offer}</span>
          </button>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center h-64 text-sky-600">
            ...
            <p className="text-slate-500 font-medium">{t.offers_page.loading}</p>
          </div>
        ) : (
              ...
              .map((offer) => (
                <OfferCard
                  ...
                  data={{
                    ...
                    status: t.offers_page.status_available as any,
                    location: offer.locationName,
                    timeAgo: new Date(offer.createdAt).toLocaleDateString(language === 'ar' ? 'ar-MA' : 'fr-MA', { day: 'numeric', month: 'short' }),
        ...
        {!loading && offers.length === 0 && (
          <div className="text-center py-20">
            ...
            <h3 className="text-lg font-bold text-slate-700">{t.offers_page.empty_title}</h3>
            <p className="text-slate-500">{t.offers_page.empty_desc}</p>
          </div>
        )}
